...
| **確認の恥辱化** | オプトアウトを悪く見せる | 「いいえ、節約したくありません」 |
| **ゴキブリホイホイ** | 入りやすく出にくい | 簡単な購読、見つけにくい解約 |
| **偽装広告** | コンテンツ/ナビに見せかけた広告 | 「おすすめ」と表示される有料掲載 |
| **注意の誘導** | 本来の操作から目を逸らす | 真の CTA 付近の派手なアニメーション |
...
| **執拗な催促** | 同じお願いのポップアップが繰り返し出る | 通知有効化の毎日のモーダル |
...
確認モーダルの多用はユーザーが内容を読まずにクリックする習慣を作る：
...
❌  <td title="全説明を見る">切り詰めテキスト</td>
    ← ネイティブの title ツールチップはタッチで表示されない
❌  ホバー時のみ表示されるメニューアイテム
✓   ホバーだけでなくフォーカスでもツールチップを表示
...
✓   3〜4 段階のシャドウをデザイントークンとして定義し一貫して使う：
...
```
❌  保存のたびに、ナビゲーションのたびに、些細な操作のたびにトースト表示
✓   明白でない結果を持つ操作にのみトーストを使う
