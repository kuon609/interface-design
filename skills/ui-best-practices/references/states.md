...
| パターン | 使用タイミング |
|---------|-------------|
| **skeleton** | 予測可能なレイアウトのページ/セクションの初期読み込み |
| **spinner** | 所要時間が不明なアクション（ボタンクリック、フォーム送信） |
| **プログレスバー** | 所要時間またはステップ数がわかっている操作（ファイルアップロード、マルチステップ） |
| **楽観的更新** | 成功する可能性が高い高速操作（いいね/解除、並べ替え） |
| **部分読み込み表示** | 読み込み済みページ内でのセカンダリデータ読み込み |

### skeleton

実際のコンテンツレイアウトに合わせて skeleton を作成する：
- 実際のコンテンツと同じ `border-radius` を使う
- シマー（`background: linear-gradient(90deg, ...)`）またはパルスアニメーションを付ける
- 約 3 秒以上 skeleton を表示する場合はフォールバックを用意する
...
| ページレベルのエラー | コンテンツエリアの上部（modal ではない） |
| アクションのエラー | toast / 通知、またはアクションの近くにインライン |
...
**一時的**（toast / 通知）：3〜5 秒後に自動消去。スワイプ/手動で閉じられる。
